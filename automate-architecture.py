...
if __name__ == "__main__":
    ...
    print("Architecture Competition !")
    
    ## LSTM
    path = "./LSTM/train.py"
    args_list = {
        ("train_data", "test_data") : [
            ("../dataset/lyrics/small-medium-large/small/train.txt", "../dataset/lyrics/small-medium-large/small/test.txt"),
            ("../dataset/lyrics/small-medium-large/medium/train.txt", "../dataset/lyrics/small-medium-large/medium/test.txt"),
            ("../dataset/lyrics/small-medium-large/large/train.txt", "../dataset/lyrics/small-medium-large/large/test.txt")
        ],
        "seq_len" : [64],
    ...
    os.makedirs("./result/architecture/LSTM", exist_ok=True)
    ...
    for i, args in enumerate(args_combo):
        ...
        run(path, args)
        shutil.copyfile("./LSTM/model-experiment/stats.json", f"./result/architecture/LSTM/{dataset_type}-{i}.json")
    ...
    os.makedirs("./result/architecture/GPT", exist_ok=True)
    args_combo = create_args_combo(args_list)
    bar = tqdm(total=len(args_combo), desc="GPT")
    for i, args in enumerate(args_combo):
        dataset_type = args["train_data"].split('/')[-2]
        run(path, args)
        shutil.copyfile("./MiniCharGPT/model-experiment/stats.json", f"./result/architecture/GPT/{dataset_type}-{i}.json")
